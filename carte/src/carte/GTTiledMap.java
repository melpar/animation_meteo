...
public class GTTiledMap extends JPanel implements RenderListener {
  ...
  public void setCenter(Coordinate newCenter) {
    ...
    for (int x = (dx > 0) ? tileMatrix.length - 1 : 0; x < tileMatrix.length
        && x >= 0; x += (dx > 0) ? -1 : 1) {
      for (int y = (dy > 0) ? tileMatrix[x].length - 1 : 0; y < tileMatrix[x].length
          && y >= 0; y += (dy > 0) ? -1 : 1) {
        int nx = x + dx;
        int ny = y + dy;
        int cx = x - dx;
        ...
        if (nx >= 0 && nx < tileMatrix.length && ny >= 0 && ny < tileMatrix[nx].length) {
          /*
        ...
        if (cx >= 0 && cx < tileMatrix.length && cy >= 0 && cy < tileMatrix[cx].length) {
          tileMatrix[x][y] = null;
        } else {
  ...
  protected void completeReset() {
    ...
    /*
     * make sure we always have an odd number of tiles that way we can make sure to
     * have a center tile
     */
    if (tiles % 2 == 0) {
      tiles++;
      ...
      for (int x = mid - z; x <= mid + z; x++) {
        for (int y = mid - z; y <= mid + z; y++) {
          if (tileMatrix[x][y] == null) {
            ...
            // + (x - tileMatrix.length / 2) * tileDimension / worldToTileScale;
            // tile.center.y = this.center.y
            // - (y - tileMatrix[x].length / 2) * tileDimension / worldToTileScale;
            queueRender(tileMatrix[x][y]);
          }
        }
      }
    ...
    if (tiles > oldTiles) {
      ...
      for (int x = 0; x < tmpTiles.length; x++) {
        for (int y = 0; y < tmpTiles[x].length; y++) {
          ...
          if (oldX >= 0 && oldX < tileMatrix.length && oldY >= 0
              && oldY < tileMatrix[oldX].length) {
            ...
            // TileContext tile = tilePool.acquire();
            // tmpTiles[x][y] = tile;
            // tile.center.x = centerX + (x - tmpTiles.length / 2) * tileDimension /
            // worldToTileScale;
            // tile.center.y = centerY
            // - (y - tmpTiles[x].length / 2) * tileDimension / worldToTileScale;
  ...
  public void setWorldToScreenScale(float _worldToScreenScale) {
    ...
    float newRadius = (float) Math
        .sqrt(this.getWidth() * this.getWidth() + this.getHeight() * this.getHeight()) / 2.0f;
  ...
  public void paint(Graphics g) {
    ...
    // System.out.println(count + " of " + tileMatrix.length * tileMatrix[0].length
    // + " drawn ");
    g2.setTransform(save);

    // g2.drawRect(-this.getWidth()/2, -this.getHeight() / 2, this.getWidth(),
    // this.getHeight());
    // g2.draw(circle);
